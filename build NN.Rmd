...
preds<-predict(base_model,batch_size = 64, x=xtrain)

prediction_matrix<-table(preds,ytrain)%>%as.data.frame
ft_auc=prediction(preds,ytrain)%>%
  performance("auc")%>%.@y.values%>%.[[1]]

prediction(preds,ytrain)%>%
  performance(measure="tpr",x.measure="fpr")%>%plot()

prediction_matrix%>%
  uncount(Freq)%>%
  mutate(preds=round(as.numeric(paste0(preds)),4))%>%
  ggplot(aes(x=preds,colour=ytrain,fill=ytrain))+
